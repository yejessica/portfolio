export default function ExperienceItem({ 
  ...
  return (
    <div className="content-stretch flex gap-[80px] items-start relative shrink-0">
        ...
        <div className="flex flex-col font-manrope font-bold justify-center relative shrink-0 text-[#3d7ba4] text-[16px]">
          {Array.isArray(company) ? (
            company.map((line, index) => (
              <p key={index} className={index === 0 ? "mb-0 leading-[24px] text-nowrap" : "leading-[24px] text-nowrap"}>
                {line}
              </p>
            ))
          ) : (
            <p className="leading-[24px] text-nowrap">{company}</p>
          )}
        </div>
        {companyUrl && (
          <a 
          ...
          >
            <p className="leading-[24px] text-nowrap">
      ...
      {/* Roles Column */}
      <div className={`content-stretch flex flex-col items-start ${roles.length > 1 ? 'gap-[12px]' : 'gap-[5px]'} justify-center leading-[0] relative shrink-0 text-nowrap`}>
